...
# -*- coding:utf-8 -*-
"""
autoTest: RequestsCheckKeywords
author: cdy
time: 2015-08-20 
"""
import os
...
class RequestsCheckKeywords(object):
    ...
    """
    split_requests_file: 分析请求日志，split截断后存入属性list: reqInitlist，请求日志文件不存在，直接返回失败
    """

    def split_requests_file(self, file):
        logger.info("------------- start %s ----------***>>>" % sys._getframe().f_code.co_name)
        if os.path.isfile(file):
            self.reqLogPath = file
            try:
                fd = open(file, "rb")
                data = fd.read()
                fd.close()
                ...
                logger.error("Open or Read reqLogFile Failed: [%s]" % file)
                return False
            self.reqSplitList = data.split("\r\n\r\n")
            return True
        else:
            logger.error("No reqLogFile: [%s]" % file)
        return False
    ...
    def ccServerCheck(self, uri, checkHeaders={}, checkMeth="GET", checkVer="1.1", checkMode="strict",
                      checkFile=reqLogPath):
        logger.info("------------- start %s ----------*****>>>" % sys._getframe().f_code.co_name)
        ...
        headerTypes = (
        types.DictType, types.ListType, types.StringType, types.UnicodeType, types.NoneType, robotDictType)
        ...
        if (type(uri) not in uriTypes):
            ...
            return -2
        if (type(checkMeth) not in otherTypes) or (type(checkVer) not in otherTypes) or (
            type(checkMode) not in otherTypes):
        ...
        res = self.split_requests_file(checkFile)
        if not res:
        ...
        for i in range(len(self.reqSplitList)):
            ...
            if uri == dictPerReq["uri"]:
                ...
                if not checkHeaders:
                    logger.warning(
                        "All matched, CheckHeaders [%s] no check, reqLine [%s] checked Succeed in reqSplitList[%s] [%s]" \
...
if __name__ == '__main__':
    obj = ccHttpServer()
    obj.split_requests_file(reqLogPath)
